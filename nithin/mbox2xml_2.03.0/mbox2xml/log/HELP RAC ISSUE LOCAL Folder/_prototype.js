...
(function(global) {
  ...
  global.Element = function(tagName, attributes) {
    ...
    var ns = attributes.xmlns || false;
    if (ns) delete attributes.xmlns;
    if (SETATTRIBUTE_IGNORES_NAME && attributes.name) {
      tagName = '<' + tagName + ' name="' + attributes.name + '">';
      delete attributes.name;
      if(ns && document.createElementNS) {
        return Element.writeAttribute(document.createElementNS(ns, tagName), attributes);
      }
      return Element.writeAttribute(document.createElement(tagName), attributes);
    }
    if (!cache[tagName + (ns || "")]) {
      if(ns && document.createElementNS) {
        cache[tagName + (ns || "")] = Element.extend(document.createElementNS(ns, tagName));
      }
      else {
        cache[tagName + (ns || "")] = Element.extend(document.createElement(tagName));
      }
    }
    return Element.writeAttribute(cache[tagName + (ns || "")].cloneNode(false), attributes);
  };
